...
public class M3uParser {
     ...
     * @return
     */
    public List<M3uEntry> parse(InputStream stream) {
        if (stream == null) {
            throw new M3uParsingException(0, "Cannot read stream");
        }
        ...
        try {
            ...
            if(line == null) {
                throw new M3uParsingException(0, "Empty stream");
            }
            lineNbr++;
            checkStart(line);
            String globalTvgShif = extract(line, TVG_SHIFT_REGEX);

            M3uEntry.Builder entry = null;
            while ((line = buffer.readLine()) != null) {
                ...
                if (isExtInfo(line)) {
                    ...
                    if (entry == null) {
                        throw new M3uParsingException(lineNbr, "Missing " + M3U_INFO_MARKER);
                    }
                    entries.add(entry.channelUri(line).build());
                }
            }
        } catch (IOException e) {
            throw new M3uParsingException(lineNbr, "Cannot read file", e);
        }
    ...
    private void checkStart(String line) {
        if (line != null) {
            if (!line.contains(M3U_START_MARKER)) {
                throw new M3uParsingException(1, "First line of the file should be " + M3U_START_MARKER);
            }
        }
    }
    ...
    private M3uEntry.Builder extractExtInfo(String globalTvgShift, String line) {
        ...
        return new M3uEntry.Builder().channelName(channelName)
                .duration(duration)
